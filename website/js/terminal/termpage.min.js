const Termpage = {
  defaultOptions: { prompt: "$", initialCommand: false, autoFocus: true },
  _appendInput: (input, options, dom) => {
    if (
      dom.$winElement.lastChild &&
      dom.$winElement.lastChild.tagName === "UL"
    ) {
  ...
  _processInput: (output, options, dom) => {
    if (output && output.then) {
      ...
      output.catch(() => {
        pre.remove();
        dom.$inputBlock.setAttribute("style", "display:flex");
        Termpage._appendOutput(
          Termpage.color("red", "command resolution failed"),
          options,
          dom
        );
      });
